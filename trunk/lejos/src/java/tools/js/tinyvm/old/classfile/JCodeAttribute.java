...
public class JCodeAttribute extends JAttribute implements IConstants
{
   ...
   public JCodeAttribute (JConstantPool aConstantPool, int aMaxStack,
      int aMaxLocals, byte aCode[], JExcepTable aExcepTable,
   ...
   public JCodeAttribute (JConstantPool aConstantPool, int aMaxStack,
      int aMaxLocals, byte aCode[], JExcepTable aTable)
   {
   ...
   {
      int pLength = 14 + getInfo().length + iExcepTable.length()
         + iAttribTable.length();
      return pLength;
   }
   ...
   public final void readInfo (InputStream aIn) throws Exception
   {
      ...
      {
         //System.out.println ("# ExcepTable.length = " +
         // iExcepTable.length());
         //System.out.println ("# AttribTable.length = " +
         // iAttribTable.length());
         //System.out.println ("# pCodeLength = " + pCodeLength);
         throw new EClassFileFormat("Attribute length mistatch: "
            + dumplength() + "/" + pDumpLength);
      }
   }

...
public abstract class JComponent implements IDumpable, IConstants
{
   ...
   public JComponent (JConstantPool aConstantPool, int aFlags, JCPE_Utf8 aName,
      JCPE_Utf8 aDescriptor) throws Exception
   {
      iConstantPool = aConstantPool;
      iFlags = aFlags;
   ...
   public JComponent (JConstantPool aConstantPool, int aFlags, String aName,
      String aDescriptor) throws Exception
   {
      this(aConstantPool, aFlags, new JCPE_Utf8(aName), new JCPE_Utf8(
         aDescriptor));
   }

   public JComponent (JConstantPool aConstantPool, int aFlags, String aName,
      Class aParamTypes[], Class aRetType) throws Exception
   {
      this(aConstantPool, aFlags, aName, "");
      iDescriptor = (JCPE_Utf8) iConstantPool.getEntry(new JCPE_Utf8(JClassName
         .getMethodDescriptor(aParamTypes, aRetType)));
   }

   public JComponent (JConstantPool aConstantPool, int aFlags, String aName,
      Class aType) throws Exception
   {
      this(aConstantPool, aFlags, aName, "");
      iDescriptor = (JCPE_Utf8) iConstantPool.getEntry(new JCPE_Utf8(JClassName
         .getDescriptorForClass(aType)));
   }

   public JComponent (JConstantPool aConstantPool, Method aMethod)
      throws Exception
   {
      this(aConstantPool, aMethod.getModifiers(), aMethod.getName(), aMethod
         .getParameterTypes(), aMethod.getReturnType());
   }

   public JComponent (JConstantPool aConstantPool, Constructor aConstructor)
      throws Exception
   {
      this(aConstantPool, aConstructor.getModifiers(), "<init>", aConstructor
         .getParameterTypes(), void.class);
   }

   public JComponent (JConstantPool aConstantPool, Field aField)
      throws Exception
   {
      this(aConstantPool, aField.getModifiers(), aField.getName(), aField
         .getType());
   }
   ...
   public void setCode (int aMaxStack, int aMaxLocals, byte aCode[])
      throws Exception
   {
      addAttribute(new JCodeAttribute(iConstantPool, aMaxStack, aMaxLocals,
         aCode));
   }

   public void setCode (int aMaxStack, int aMaxLocals, byte aCode[],
      JExcepTable aExcepTable, JAttributeTable aAttribTable) throws Exception
   {
      addAttribute(new JCodeAttribute(iConstantPool, aMaxStack, aMaxLocals,
         aCode, aExcepTable, aAttribTable));
   }

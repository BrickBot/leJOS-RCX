...
/**
 * Instances of JClassFile represent a memory image of a parsed class file. The
 * structure of a JClassFile instance consists of flags (int), fields (JTable of
 * JComponent), methods (JTable of JComponent), interfaces (JTable of
 * JCP_Index), attributes (JTable of JAttribute) and a constant pool, i.e. an
 * instance of JConstantPool. An instance of JClassFile can be constructed, and
 * JClassFile.read() can be invoked to read and parse a class file. The internal
 * structure of a JClassFile instance can also be constructed manually by
 * invoking some of its methods. The JClassFile instance can then be turned into
 * a class file by invoking JClassFile.dump().
 */
...
public class JClassFile implements IDumpable, IConstants
{
   ...
   public JCPE_Methodref getConstructorref (Class aClass, Constructor aCons)
      throws Exception
   {
      return (JCPE_Methodref) iConstantPool.getEntry(new JCPE_Methodref(
         iConstantPool, aClass, aCons));
   }
   ...
   private void readMagicAndVersions (InputStream aIn) throws Exception
   {
      ...
      if (pMinorVersion != kMinorVersion || pMajorVersion != kMajorVersion)
         throw new EClassFileFormat("Version not recognized: " + pMajorVersion
            + "." + pMinorVersion);
   }
   ...
   public String toString ()
   {
      return "Major Version: " + kMajorVersion + "\n" + "Minor Version: "
         + kMinorVersion + "\n" + "Flags: " + iFlags + "\n" + "This Class: "
         + iThisClass.toString() + "\n" + "Superclass: "
         + iSuperClass.toString() + "\n" + "Interfaces: "
         + iInterfaces.toString() + "\n" + "Fields:\n" + iFields.toString()
         + "\n" + "Methods:\n" + iMethods.toString() + "\n" + "Attrubutes:\n"

 ...
 *  mkimg.c
 *
 *  A program to make an image java class file given an s-record file.
 *
...
int main (int argc, char **argv)
{
    ...
    printf("package js.tools;\n\n");
    printf("public interface FastImage {\n\n"); 
    printf("/* Image file generated from %s by %s. */\n", argv[1],  argv[0]);
    printf("/* See source for %s for license info. */\n", argv[1]);
    printf("\n");
    printf("public static final byte [] %sImage = {", image_name);
    for (i = 0; i < image_len; i++) {
	if (i % 6 == 0)
	    printf("\n    ");
	printf("(byte) %3d", image[i]);
        if (i != image_len-1) printf(", ");
    }
    printf("\n};\n\n}\n");

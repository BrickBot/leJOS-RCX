...
public class JMisc
{
   ...
   public static int findCharNoCase (String aText, int aStart, char aChar)
   {
      int pTLen = aText.length();
      for (int pT = aStart; pT < pTLen; pT++)
         if (Character.toUpperCase(aText.charAt(pT)) == Character
            .toUpperCase(aChar))
            return pT;
      return -1;
   }

   public static int findSubStr (String aText, int aStart, String aSubStr,
      boolean aCase)
   {
      ...
      while (pT < pTLen)
      {
         pT = aCase? findChar(aText, pT, pFirst) : findCharNoCase(aText, pT,
            pFirst);
         if (pT == -1)
            return -1;
         int pIndex = pT + 1;

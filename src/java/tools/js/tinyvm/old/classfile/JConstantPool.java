...
public class JConstantPool implements IDumpable, IConstantPool
{
   ...
   /**
    * Given an instance of IConstantPoolEntry, this method tries to find the
    * entry in the constant pool (i.e. this). If an equivalent entry already
    * exists, the existing entry is returned. If not, the new entry is appended
    * to the constant pool, and it is returned. If the type of the entry is
    * JCPE_Double or JCPE_Long, an additional JCPE_Dummy entry is appended
    * (TJVMS 4.4.5).
    */
   ...
   /**
    * Updates each entry after they have been read in. An update normally
    * consists of converting a constant-pool index into an instance of
    * JConstantPoolEntry.
    */
   ...
   private final void hashEntryVector () throws Exception
   {
      ...
      for (int pIndex = 0; pIndex < pSize; pIndex++)
      {
         ...
         if (iHashtable.contains(pObj))
            throw new EClassFileFormat("Duplicate Entry # " + pIndex + " : "
               + pObj);

...
public class RCXBean implements java.io.Serializable {
   ...
   private void openComPort() throws IOException {
      ...
      System.gc();
      if (comPort == null) throw new IOException("COM port not defined");
   ...
   * @param value A identifier for the comPort e.g. <CODE>COM1</CODE>
   * or <CODE>/dev/ttyS0</CODE>
   ...
   */
/*  public byte[] getMessage(int n) throws IOException {
    byte[] b = new byte[n];
    port.setListen(true);
    try {
      for (int i=0; i<n; i++)
...
*/
   ...
   */
/*  public void setMessage(byte[] b) throws IOException {
    if (!open) openComPort();
    out.write (b);
    out.flush ();
  }
*/
   ...
   * The integer is sent as four bytes.
 ...
 * The RCX bean can not be locked by an other object, before the <code>free()</code>
 * has bin run.<br>
 * <B>Caution!!<b> The lock does not prevent other thread from using the RCX bean!
 * @param o The lock is bound to this object. It should therefore be unique for the thread.

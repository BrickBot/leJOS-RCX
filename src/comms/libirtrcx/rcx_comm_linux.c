 ...
 *
 *  03/03/2005 Matthias Paul Scholz read tty from env variable RCX_PORT at need
...
rcx_dev_t *__rcx_open(char *tty, int is_fast)
{
	...
	/* if tty is not set, read it from env variable RCX_PORT */
	/* if RCX_PORT is not set also, return NULL */
	if(strlen(tty)==0) {
		tty = getenv("RCX_PORT");

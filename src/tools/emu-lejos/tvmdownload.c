/**
 * tvmdownload.c
 * Adapded from Kekoa Proudfoot's firmdl.c,
 * described in comment below. License also
 * below.
 */
...
main(int argc, char **argv)
{
    ...
    if ((binFileFd = open(argv[1], O_RDONLY)) == -1) {
    ...
    rcx_send (fd, send, 5, recv, 1);

#if 0
    for (;;)
    {
      numRead = read (binFileFd, send, 6);
      printf ("Read %d bytes...\n", numRead);
      if (numRead <= 0)
        break;
      printf ("sending: %d, %d ...\n", send[0], send[1]);
      if (rcx_send (fd, send, numRead, recv, 1) == 1)
      {
        printf ("rcx_send returned -1");
        break;
      }
    }
#endif
